...
class LanguageTranslationController extends Controller
{
    ...
    public function index(Request $request, $language)
    {
        ...
        $groups_arr = $this->translation->getGroupsFor(config('app.locale'))->merge('single')->toArray();
        $groups = $groups_arr;
        for ($i=0;count($groups_arr) > $i;$i++) {
            if($groups_arr[$i] == "auth" || $groups_arr[$i] == "single" || $groups_arr[$i] == "validation" || $groups_arr[$i] == "passwords" || $groups_arr[$i] == "pagination" || $groups_arr[$i] == "translation::translation" || $groups_arr[$i] == "translation::errors" || $groups_arr[$i] == "errors" || $groups_arr[$i] == "translation") {
                unset($groups[$i]);
            }
        }
